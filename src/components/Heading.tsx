...
const Heading = ({ size, children, color = '', className = '' }: PropsWithChildren<HeadingProps>) => {
    ...
    switch (size) {
        ...
        case 'h2':
            return <h2 className={cn({'text-[22px]': !className}, className)} style={style}>{children}</h2>
        case 'h3':
            return <h3 className={cn('text-3xl', className)} style={style}>{children}</h3>
        case 'h4':
            return <h4 className={cn({'text-2xl': !className}, className)} style={style}>{children}</h4>
        case 'h5':
